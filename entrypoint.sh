...
[[ $1 == *"kafka"* ]] && {
    ...
    [[ ! -z "$ZK_CHROOT" ]] && {
        ...
        sed -r -i "s/(zookeeper.connect)=(.*)/\1=${ADVERTISED_HOST}:2181\/$ZK_CHROOT/g" ${KAFKA_HOME}/config/server.properties
    } || {
        # configure kafka
        sed -r -i "s/(zookeeper.connect)=(.*)/\1=${ADVERTISED_HOST}:2181/g" ${KAFKA_HOME}/config/server.properties
    }
